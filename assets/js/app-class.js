window.addEventListener('firebase-ready', () => {
    ...
    // Thuật toán nén ảnh: W_400 (rộng 400px), q_auto (chất lượng tự động), f_auto (định dạng tự động)
    const getThumb = (url) => {
        ...
        target.innerHTML = `
            ...
            <div style="display: flex; flex-wrap: wrap;">
                ...
                <div class="modal-right" style="flex: 1; padding: 40px;">
                    <h2 style="font-family: var(--font-title); font-size: 3.5rem; color: #111;">${student.name}</h2>
                    <div style="display: flex; gap: 10px; margin: 20px 0;">
                        ${student.personality?.map(p => `<span style="padding: 5px 15px; background: #f3f4f6; border-radius: 20px; font-size: 0.85rem; font-weight: 700;">#${p}</span>`).join('')}
                    </div>
                    <div style="margin-top: 30px;">
                        <div style="font-size: 0.7rem; font-weight: 800; letter-spacing: 3px; color: var(--sakura-pink); margin-bottom: 15px;">DẤU ẤN RỰC RỠ</div>
                        <ul style="list-style: none; padding: 0;">
                            ${student.achievements?.map(a => `<li style="margin-bottom: 10px; font-size: 1.1rem; display: flex; align-items: center; gap: 10px;"><div style="width:6px; height:6px; background:var(--sakura-pink); border-radius:50%;"></div> ${a}</li>`).join('')}
                        </ul>
                    </div>
                    <div style="margin-top: 40px; padding: 30px; background: #fffcf0; border-radius: 15px; border: 1px dashed #d4af37; position: relative;">
                        <i data-lucide="quote" style="position: absolute; top: 10px; left: 15px; opacity: 0.1; width: 40px; height: 40px;"></i>
                        <p class="handwritten" style="font-size: 2rem; color: #1d4ed8; text-align: center;">"${student.quote}"</p>
                        <p style="font-size: 0.9rem; font-style: italic; color: #666; margin-top: 20px; text-align: center; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 15px;">Kỷ niệm riêng: ${student.secretMemory}</p>
                    </div>
                </div>
            </div>
        `;
    ...
    musicBtn.addEventListener('click', () => {
        if (isPlaying) {
            ...
            musicStatus.innerText = "Đang phát: Những Ngày Năm Ấy";
        }
        isPlaying = !isPlaying;
        lucide.createIcons();
    });
