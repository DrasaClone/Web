const { initializeApp, getAuth, GoogleAuthProvider, signInWithPopup, signOut, onAuthStateChanged, getDatabase, ref, set, get } = window.fb;
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (snapshot.exists()) {
            ...
            document.getElementById('avatar-preview').innerHTML = `<img src="${data.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
            
            // Hiện nút Xem thành quả
...
document.getElementById('reg-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Nếu có chọn file mới thì mới upload Cloudinary
        if (file) {
        ...
        alert("Đã lưu hồ sơ thành công! Cậu có thể xem trang lớp rồi đó.");
        document.getElementById('view-class-btn').style.display = 'block';
    } catch (err) {
...
document.getElementById('avatar-input').addEventListener('change', (e) => {
    ...
    if (file) {
        document.getElementById('avatar-preview').innerHTML = `<img src="${URL.createObjectURL(file)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
    }
});
