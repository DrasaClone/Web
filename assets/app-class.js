const { initializeApp, getAuth, onAuthStateChanged, getDatabase, ref, onValue, push } = window.fb;
        ...
        Object.values(data).forEach(student => {
            ...
            card.innerHTML = `
                ...
                <div style="text-align: center; margin-top: 10px;" class="mono">
                    <span style="font-size: 0.6rem; color: var(--sakura-primary);">XEM KỶ NIỆM</span>
                </div>
            `;
...
onValue(ref(db, 'class_2026/wishes'), (snapshot) => {
    ...
    if (data) {
        Object.values(data).forEach(wish => {
            ...
            gsap.to(item, { x: "random(-50, 50)", y: "random(-50, 50)", duration: "random(5, 10)", repeat: -1, yoyo: true, ease: "sine.inOut" });
        });
    }
});
...
function showDetail(student) {
    ...
    content.innerHTML = `
        <img src="${student.avatar}" style="width:100%; height:350px; object-fit:cover; border-radius:25px; margin-bottom:30px;">
        ...
        <div style="margin-bottom:20px;">
            <div class="mono" style="color:var(--sakura-primary); margin-bottom:10px; font-weight:700;">THÀNH TỰU</div>
            <ul style="list-style:none; padding:0;">
                ${student.achievements?.map(a => `<li style="margin-bottom:8px; font-size:1.1rem;">• ${a}</li>`).join('')}
            </ul>
        </div>
        <div style="margin-top:30px; padding:30px; background:var(--bg-soft); border-radius:30px; position:relative; border: 1px dashed var(--sakura-light);">
            <p class="handwritten" style="font-size:2rem; color:#3b82f6; text-align:center;">"${student.quote}"</p>
            <p style="font-size:0.95rem; font-style:italic; color:#777; margin-top:20px; text-align:center; border-top:1px solid rgba(0,0,0,0.05); padding-top:15px;">Kỷ niệm: ${student.secretMemory}</p>
        </div>
    `;
